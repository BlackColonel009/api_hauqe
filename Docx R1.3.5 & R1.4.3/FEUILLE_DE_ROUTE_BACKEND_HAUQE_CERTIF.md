...
- API CRUD des zones administratives hiérarchiques et route de création rapide Collecte ;
- endpoint `POST /api/v1/collectes/quick-enterprises` pour dossier entreprise incomplet terrain ;
- statut `INCOMPLET_COLLECTE`, source `COLLECTE_TERRAIN`, audit et anti-doublon exact ;
- colonne `certifications_declarees.situation_declaree` avec migration SQL et validation serveur ;
- création utilisateur et rôles dans une transaction unique ;
- protection SNCC : résultat INFC validé obligatoire avant classement/reclassement ;
